...
head(data.cola.agg)
colnames(data.cola.agg)
data.cola.agg$share1<-data.cola.agg$units/data.cola.agg$sumunits*(data.cola.agg$sumdollars/data.cola.agg$total_rev_cola)
summary(data.cola.agg$share1)
# share1<-units/sumunits*(sumdollars/total_rev_cola)
# This is NOT a withinshare in a nested-logit-sense!
# He just uses this to calculate the share of the outside good.
...
summary(data.cola.agg$shareog1)
...
d$price.sim<-d$price-d$DC.price
...
#pred share if all brands cut the price by 20 percent


d$all.price<-d$price*.2
d$price.sim<-d$price-d$all.price
...
tmp<-rename(tmp, c("expdelta.ALL.8"="sumexpdelta.ALL.8"))
...
d$delta.tmp<-d$CC*est['b1']+d$DC*est['b2']+d$P*est['b4']+d$DP*est['b3']+d$price.sim*est['alpha']
d$expdelta.tmp<-exp(d$delta.tmp)
tmp<-aggregate(cbind(expdelta.tmp) ~ MARKET+CHAIN+store_type+week, data = d, sum, na.rm = TRUE)
tmp<-rename(tmp, c("expdelta.tmp"="sumexpdelta.tmp"))

d <- merge(d,tmp,by=c("MARKET","CHAIN","store_type","week"))
head(d)
d$predshare.tmp<-d$expdelta.tmp/(1+d$sumexpdelta.tmp)





tmp<-aggregate(cbind(predshare,predshare.tmp,predshare.tmp) ~ MARKET+CHAIN+store_type+L4+L5, data = d, mean, na.rm = TRUE)
tmp$rtmp<-tmp$predshare.tmp/tmp$predshare
head(tmp)




# Simulation with nested logit model
...
summary(data.cola.agg$share.diet)

nt<-lm(log(share1)-log(shareog1)~-1+log(share.diet)+L5+yearfactor+(price)+displayall+featureall,data=data.cola.agg)
summary(nt)
...
tmp<-rename(tmp, c("nom.Dg"="denom.Dg"))
head(tmp)
d <- merge(d,tmp,by=c("MARKET","CHAIN","store_type","week"))
head(d)
...
# pred shares at price reduction of DC  by 20 percent
